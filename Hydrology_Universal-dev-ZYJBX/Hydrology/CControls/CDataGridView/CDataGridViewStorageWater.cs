...
{
     class CDataGridViewStorageWater : CExDataGridView
    {
        #region 静态常量
...
#pragma warning disable CS0414 // 字段“CDataGridViewStorageWater.CS_OnlineOrOffline”已被赋值，但从未使用过它的值
        private static readonly string CS_OnlineOrOffline = "在线状态";
#pragma warning restore CS0414 // 字段“CDataGridViewStorageWater.CS_OnlineOrOffline”已被赋值，但从未使用过它的值
        private static readonly string CS_StorageWater = "存储水位";
...
#pragma warning disable CS0169 // 从不使用字段“CDataGridViewStorageWater.m_itemOfflineFirst”
        private ToolStripMenuItem m_itemOfflineFirst;
#pragma warning restore CS0169 // 从不使用字段“CDataGridViewStorageWater.m_itemOfflineFirst”
        private ToolStripMenuItem m_itemRecoverDefault; //恢复默认
    

        public CDataGridViewStorageWater()
            : base()
        {
            InitDataGridView();
            m_listSubCenter = CDBDataMgr.GetInstance().GetAllSubCenter();
            CProtocolEventManager.DownForUI += this.DownForUI_EventHandler_1;
        }
        ...
        public void SetSubCenterName(string subcenterName)
        {
            if (subcenterName == null || subcenterName.Equals(""))
            {
                ...
                List<CEntityStation> listStorageWaterStation=new  List<CEntityStation>();
                //所有水位站和水文站，0，2
                for (int i = 0; i < listStation.Count; i++)
                {
                    if(listStation[i].StationType==EStationType.EHydrology||listStation[i].StationType==EStationType.ERiverWater)
                    {
                        listStorageWaterStation.Add(listStation[i]);
                    }
                }
                SetStationData(listStorageWaterStation);
            }
            else
            {
                // 根据分中心查找测站
        ...
        public bool UpdateStorage(string stationId, string data)
        {
            ...
            for (int i = 0; i < m_dataTable.Rows.Count; ++i)
            {
                if (stationId == m_dataTable.Rows[i][CS_StationID].ToString())
                {
                    ...
                    //}
                    string parsedData = float.Parse(data.Substring(0, 4)).ToString() + "." + data.Substring(4, 2).ToString();
        ...
        public void EHOnlineFirst(object sender, EventArgs e)
        {
            ...
            {
                // 排序
                this.Sort(this.Columns[CS_StorageWater], System.ComponentModel.ListSortDirection.Descending);
            }
        }
        ...
        public void EHRecoverDefault(object sender, EventArgs e)
        {
            ...
            {
                // 排序
                this.Sort(this.Columns[CS_StorageWater], System.ComponentModel.ListSortDirection.Ascending);
